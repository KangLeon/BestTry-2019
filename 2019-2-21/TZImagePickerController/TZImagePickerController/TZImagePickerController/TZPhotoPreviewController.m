...
#pragma mark - tz修改
    ...
    [_doneButton setTitleColor:[UIColor colorWithRed:0.071 green:0.471 blue:0.843 alpha:1.00] forState:UIControlStateNormal];
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            if (_tzImagePickerVc.maxImagesCount == 20) {
                NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"单次最多选择20张图片上传"], _tzImagePickerVc.maxImagesCount];
