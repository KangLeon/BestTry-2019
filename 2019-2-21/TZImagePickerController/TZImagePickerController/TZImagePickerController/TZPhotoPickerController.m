...
- (void)configBottomToolBar {
    ...
    [_doneButton setTitleColor:[UIColor colorWithRed:0.675 green:0.675 blue:0.675 alpha:1.00] forState:UIControlStateNormal];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                if (tzImagePickerVc.maxImagesCount == 20) {
                    NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"单次最多选择20张图片上传"], tzImagePickerVc.maxImagesCount];
                    [tzImagePickerVc showAlertWithTitle:title];
                }else{
