...
@property (nonatomic, assign)BOOL showSingleTap;
@end
...
- (UILabel *)tagText{
    if (_tagText == nil) {
        ...
        _tagText.frame = CGRectMake(20, SCREEN_HEIGHT - 60, SCREEN_WIDTH - 40, 30);
        _tagText.text = @"";
        _tagText.textColor = [UIColor whiteColor];
        if (@available(iOS 8.2, *)) {
            _tagText.font = [UIFont systemFontOfSize:14 weight:1.5];
        } else {
...
- (UIButton *)backBtn{
    if (_backBtn == nil) {
        ...
        if (KIsiPhoneX) {
            _backBtn = [[UIButton alloc]initWithFrame:CGRectMake(10, 20+20, 40, 40)];
        }else{
        ...
        [_backBtn setImage:[UIImage imageNamed:@"back_icon"] forState:UIControlStateSelected];
        [_backBtn setImage:[UIImage imageNamed:@"back_icon"] forState:UIControlStateHighlighted];
        [_backBtn addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchDown];
    }
    return _backBtn;
}
...
- (UIButton *)deleteBtn{
    if (_deleteBtn == nil) {
        ...
        if (KIsiPhoneX) {
            _deleteBtn.frame = CGRectMake(SCREEN_WIDTH - 50, 20+20, 40, 40);
        }else{
            _deleteBtn.frame = CGRectMake(SCREEN_WIDTH - 50, 20, 40, 40);
        }
        [_deleteBtn setImage:[UIImage imageNamed:@"rubbish"] forState:UIControlStateNormal];
        [_deleteBtn setImage:[UIImage imageNamed:@"rubbish"] forState:UIControlStateSelected];
        [_deleteBtn setImage:[UIImage imageNamed:@"rubbish"] forState:UIControlStateHighlighted];
...
//    [self addSubview:indexLabel];
    ...
    if (self.pageControlStyle == XLPhotoBrowserPageControlStyleNone) {
        [self showTodoStyle];
    }
...
//            [self addSubview:pageControl];
            ...
            break;
        case XLPhotoBrowserPageControlStyleNone:
        {
            UIPageControl *pageControl = [[UIPageControl alloc] init];
            _pageControl = pageControl;
            pageControl.numberOfPages = self.imageCount;
...
- (void)deleteAction{
    ...
    if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(setDeleteDelegateCurrentImageIndex:)]) {
        [weakSelf.delegate setDeleteDelegateCurrentImageIndex:weakSelf.currentImageIndex];
    }
}
