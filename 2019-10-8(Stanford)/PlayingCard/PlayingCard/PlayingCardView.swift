...
class PlayingCardView: UIView {
    ...
    var rank: Int = 12 {
        didSet {
            setNeedsDisplay()//->系统会调用draw()
            setNeedsLayout()//->系统会调用layoutSubviews()
        }
    }
    @IBInspectable
    var suit: String = "♥️" {
    ...
    @objc func adjustFaceCardScale(buHandlingGestureRecognizedBy recognizer: UIPinchGestureRecognizer) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            ...
            if let faceCardImage = UIImage(named: "cardback", in:Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
