...
//                   [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"wss://weixin.csmc-cloud.com/livehappyprd/wss/exercise-server/barrageCenter/socket/123456789",host]]]];//这里填写你服务器的地址
    
    self.socket = [[SRWebSocket alloc] initWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"wss://weixin.csmc-cloud.com/livehappyprd/wss/exercise-server/barrageCenter/socket/123456789"]]];
...
- (void)initHeartBeat
{
    dispatch_main_async_safe(^{
        ...
        heartBeatTimer = [NSTimer timerWithTimeInterval:30 target:self selector:@selector(ping) userInfo:nil repeats:YES];
...
- (void)destoryHeartBeat
{
    dispatch_main_async_safe(^{
        if (heartBeatTimer) {
            if ([heartBeatTimer respondsToSelector:@selector(isValid)]){
