...
- (void)appendImage:(UIImage *)image alignment:(HLTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
    ...
    CGFloat offsetY = kHLPadding;
    CGFloat imageX = (kHLPreviewWidth - newImage.size.width) * 0.5;
    imageView.frame = CGRectMake(imageX, _height, newImage.size.width, newImage.size.height);
    _height += (offsetY + newImage.size.height);
